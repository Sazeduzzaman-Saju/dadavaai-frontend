...
$(document).ready(function () {
    ...
    function checkRequiredFields() {
      ...
      $("#myForm input[required], #myForm textarea[required]").each(
        function () {
          var fieldName = $(this).attr("name");
          var fieldValue = $.trim($(this).val());
          var errorContainer = $("#" + fieldName + "-error");
          var label = $("label[for='" + fieldName + "']");
          var labelText = label.text().replace(/\*/, ""); // Remove the asterisk (*) from label text

          if (fieldValue === "") {
            isValid = false;
            $(this).closest(".form-group").addClass("has-error"); // Add the Bootstrap error class to the parent form-group
            errorContainer.text(labelText + " is required.");
          } else {
            $(this).closest(".form-group").removeClass("has-error"); // Remove the Bootstrap error class from the parent form-group
            errorContainer.text("");
          }
        }
      );
    ...
    // Trigger form validation on form submission
    $("#myForm").on("submit", function (event) {
      event.preventDefault();
      var isFormValid = checkRequiredFields();
    ...
    // Trigger form validation on input change and focus out
    $("#myForm input[required], #myForm textarea[required]").on(
      "input blur",
      function () {
        var isFormValid = checkRequiredFields();
    ...
    // Hide error message when focusing on a required field
    $("#myForm input[required], #myForm textarea[required]").on(
      "focus",
      function () {
        var fieldName = $(this).attr("name");
  ...
  // Form  Validation for billing user dashboard
$(document).ready(function () {
    // Function to check if all required fields are filled
    function checkRequiredFields() {
      var isValid = true;
      ...
      // Check each required field
      $("#myForm-billing input[required], #myForm-billing textarea[required]").each(
        function () {
          var fieldName = $(this).attr("name");
          var fieldValue = $.trim($(this).val());
          var errorContainer = $("#" + fieldName + "-error");
    ...
    // Trigger form validation on form submission
    $("#myForm-billing").on("submit", function (event) {
      event.preventDefault();
      var isFormValid = checkRequiredFields();
      ...
      if (isFormValid) {
        ...
        $("#submit-btn-billing").prop("disabled", true);
      }
    });

    // Trigger form validation on input change and focus out
    $("#myForm-billing input[required], #myForm-billing textarea[required]").on(
      "input blur",
      function () {
        var isFormValid = checkRequiredFields();
        $("#submit-btn-billing").prop("disabled", !isFormValid);
      }
    );
...
// Form  Validation for shipping user dashboard
$(document).ready(function () {
    // Function to check if all required fields are filled
    function checkRequiredFields() {
      var isValid = true;
      ...
      // Check each required field
      $("#myForm-shipping input[required], #myForm-shipping textarea[required]").each(
        function () {
          var fieldName = $(this).attr("name");
          var fieldValue = $.trim($(this).val());
          var errorContainer = $("#" + fieldName + "-error");
    ...
    // Trigger form validation on form submission
    $("#myForm-shipping").on("submit", function (event) {
      event.preventDefault();
      var isFormValid = checkRequiredFields();
      ...
      if (isFormValid) {
        ...
        $("#submit-btn-shipping").prop("disabled", true);
      }
    });

    // Trigger form validation on input change and focus out
    $("#myForm-shipping input[required], #myForm-shipping textarea[required]").on(
      "input blur",
      function () {
        var isFormValid = checkRequiredFields();
        $("#submit-btn-shipping").prop("disabled", !isFormValid);
      }
    );
...
// for checkout page
$(document).ready(function() {
    // Get references to the checkboxes and the "Proceed to Checkout" button
    var billingCheckbox = $('#billingCheckbox');
    var shippingCheckbox = $('#shippingCheckbox');
    var billingInfoArea = $('#billingInfoArea');
    var shippingInfoArea = $('#shippingInfoArea');
    var checkoutButton = $('.checkout-button');

    // Disable the "Proceed to Checkout" button initially
    checkoutButton.prop('disabled', true);

    // Enable/disable the button based on checkbox states
    function updateCheckoutButton() {
      if (billingCheckbox.is(':checked') || shippingCheckbox.is(':checked')) {
    ...
    // Add change event listener to the billing checkbox
    billingCheckbox.change(function() {
      if (billingCheckbox.is(':checked')) {
        shippingCheckbox.prop('checked', false); // Uncheck the shipping checkbox
        shippingInfoArea.hide();
      }
      billingInfoArea.toggle();
      updateCheckoutButton();
    });

    // Add change event listener to the shipping checkbox
    shippingCheckbox.change(function() {
      if (shippingCheckbox.is(':checked')) {
        billingCheckbox.prop('checked', false); // Uncheck the billing checkbox
        billingInfoArea.hide();
      }
      shippingInfoArea.toggle();
      updateCheckoutButton();
    });

    // Submit the form when the "Proceed to Checkout" button is clicked
    checkoutButton.click(function() {
      $('form').submit();
    });

    // Hide the shipping address area by default
    shippingInfoArea.hide();

    // Show the billing info area by default
    billingInfoArea.show();
  });
